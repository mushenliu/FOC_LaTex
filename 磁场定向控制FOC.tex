...
\newcommand{\dd}{\mathrm{d}}  %微分正体d
\newcommand{\ee}{\mathrm{e}}  %自然常数正体e
\newcommand{\jj}{\mathrm{j}}  %虚数单位正体j
...
\makeatletter
\let\oldcite\cite
\def\cite{\@ifnextchar[\cite@opt\cite@plain}
\def\cite@opt[#1]#2{\textsuperscript{\oldcite[#1]{#2}}}
\def\cite@plain#1{\textsuperscript{\oldcite{#1}}}
\makeatother
...
\renewcommand{\thefootnote}{[\ifnum\value{footnote}<26\Alph{footnote}\else\arabic{footnote}\fi]}
% 定义样式
...
%正文页
\input{章节/第一章：永磁同步电机的数学模型.tex}

\input{章节/第二章：SVPWM调制技术.tex}

\input{章节/第三章：经典FOC的速度-电流双闭环PI控制实现.tex}

\input{章节/第四章：永磁同步电机的参数辨识.tex}

\input{章节/第五章：速度-电流环的频域矫正控制器设计.tex}

\input{章节/第六章：最大转矩电流比控制MTPA.tex}
...
\input{章节/附录A：线性代数与离散系统分析基础知识.tex}

\input{章节/附录B：Ho-Kalman辨识算法.tex}

\input{章节/附录C：基于维纳-霍普夫方程的脉冲响应估计质量分析.tex}

\input{章节/附录D：牛顿迭代法.tex}
