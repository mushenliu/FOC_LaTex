...
此时电磁转矩为\(T_{\mathrm{e}}=\cfrac{3}{2}p\psi_{\mathrm{m}}i_{\mathrm{q}}\propto i_{\mathrm{q}}\)为增益环节。而Q轴反电动势为
\(E=\omega_{\mathrm{e}}(\psi_{\mathrm{m}}+L_{\mathrm{d}}i_{\mathrm{d}})\)，在\(i_{\mathrm{d}}=0\)时可以简化为\(E=\omega_{\mathrm{e}}\psi_{\mathrm{m}}\propto \omega_{\mathrm{e}}\)也是线性环节。
...
根据上图可知，在\(i_{\mathrm{d}}=0\)的理想化假设下，Q轴电流和转速环由于反电动势构成了物理特性导致的闭环，使得被迫在闭环情况下
进行辨识，这会对系统辨识产生很大的影响。相关推导见\ref{sec:基于相关分析的Ho-Kalman辨识偏倚性分析}。根据
\eqref{eq:闭环有偏性公式}可知，在闭环情况下估计的脉冲响应序列对应的离散时间序列傅里叶变换DTFT满足：
\begin{equation*}
    \hat{G}(e^{\mathrm{j}\omega})=G(e^{\mathrm{j}\omega})[1-\cfrac{\sigma_{x}^2}{S_{xx}(e^{\mathrm{j}\omega})}]
    \tag{\ref{eq:闭环有偏性公式}}
\end{equation*}

然而，真实的\(G(e^{\mathrm{j}\omega})\)未知，偏差因子\(\cfrac{\sigma_{x}^2}{S_{xx}(e^{\mathrm{j}\omega})}\)也无法从辨识数据中直接计算。
...
与电流环控制器一样，采用一阶超前校正控制\(C_{\mathrm{s}}(s)=K\cfrac{Ts+1}{\alpha Ts+1}\)，开环截止频率设定为12Hz，
...
\begin{equation*}
    C_{\mathrm{s}}(s)=\cfrac{0.0007477s+0.0125}{0.00294s^2+s}
\end{equation*}

使用双线性变换离散化后得到：
\begin{equation*}
    C_{\mathrm{s}}(z)=\cfrac{0.0001096+1.817\times 10^{-6}z^{-1}-0.0001078z^{-2}}{1-1.7093z^{-1}+0.7093z^{-2}}
\end{equation*}
