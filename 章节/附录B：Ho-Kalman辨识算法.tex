...
\begin{itemize}
    ...
    \item 级数\(N\)：级数越高，伪随机在频域上的频点间隔越小，但相应的每个频点上的激励能力下降。而级数\(N\)也影响着辨识时长。对于大部分
    机电系统，推荐选取\(N\)=9\(\sim\)12。
\end{itemize}
...
\footnote{若有时域信号\(x\)和\(y\)，相关运算指的是进行\(R_{\mathrm{xy}}(\tau)=\text{E}[x(t)y(t+\tau)]\)，
...
\(R_{\mathrm{xy}}(\tau)=\text{E}[x(t)]\cdot \text{E}[y(t+\tau)]\)为两者均值之积。
}
\begin{equation*}
    R_{xx}(k)=\cfrac 1M\sum_{i=0}^{M-1} x_i x_{i+k} =\cfrac 1M(x_{{-k}} * x_{k})
\end{equation*}

输入与输出信号的\textbf{互相关序列（Cross-Correlation Sequence）}：
\begin{equation*}
    R_{xy}(k)=\cfrac 1M\sum_{i=0}^{M-1} x_i y_{i+k} =\cfrac 1M(x_{{-k}} * y_{k})
\end{equation*}

其中\(x_{-k}\)表示输入序列\(x_k\)的时间反转。
...
定义向量
\(\bm{r}_{xy}=\begin{bmatrix}
        R_{xy}(0)\\
        R_{xy}(1)\\
        \vdots\\
        R_{xy}(N-1)
    \end{bmatrix}\)
和\(\bm{g}=\begin{bmatrix}
        g_0\\
        g_1\\
        \vdots\\
        g_{N-1}
...
\begin{equation*}
    \bm{r}_{xy}=\bm{r}_{xx}\bm{g}
\end{equation*}

脉冲响应序列可以通过矩阵求逆计算得到：

\begin{equation}
    \bm{g}=\bm{r}_{xx}^{-1}\bm{r}_{xy}
    \label{eq:脉冲响应序列求解}
\end{equation}
...
得到脉冲响应序列后，可以按照\ref{eq:汉克尔矩阵}的定义构造系统的汉克尔矩阵。根据离散系统分析的相关知识，若系统本身是\(n\)阶的，
那么汉克尔矩阵奇异值分解后会有\(n\)个主导奇异值。若系统本身是\(m\)阶的，那么理想情况下只要构造的汉克尔矩阵阶次\(n>m\)即可包含全部的模态。
...
而构造一个\(n\)阶的汉克尔矩阵，需要用到\(g_1\)到\(g_{2n-1}\)的信息。据图求出暂态响应基本结束的时刻（\ref{fig:脉冲响应序列}为20），将其
...
至此已经得到了系统的汉克尔矩阵\(\bm{\mathcal{H}}\)，
并完成了奇异值分解得到\(\bm{\mathcal{H}}=\bm{U}\bm{\Sigma}\bm{V}^T\)，也知道了系统的阶次。如何求解
最小状态空间实现需要的\(\bm{A}\)，\(\bm{B}\)，\(\bm{C}\)，\(\bm{D}\)是接下来讨论的内容。
...
\begin{equation*}
    ...
    =
    \begin{bmatrix}
        \bm{CB} & \bm{CAB} & \cdots & \bm{CA}^{n-1}\bm{B} \\
        \bm{CAB} & \bm{CA}^2\bm{B} & \cdots & \bm{CA}^n\bm{B} \\
        \vdots & \vdots & \ddots & \vdots \\
        \bm{CA}^{n-1}\bm{B} & \bm{CA}^n\bm{B} & \cdots & \bm{CA}^{2n-2}\bm{B} \\
    \end{bmatrix}
    =
    \begin{bmatrix}
        \bm{C}\\\bm{CA}\\\bm{CA}^2\\\bm{\vdots}\\\bm{CA}^{n-1}
    \end{bmatrix}
    \begin{bmatrix}
        \bm{B}&\bm{AB}&\bm{A}^2\bm{B}&\bm{\cdots}&\bm{A}^{n-1}\bm{B}
    \end{bmatrix}
     =\bm{\mathcal{O}\mathcal{C}}
\end{equation*}

也即汉克尔矩阵是能控性矩阵\(\bm{\mathcal{C}}\)与能观性矩阵\(\bm{\mathcal{O}}\)之积，
结合汉克尔矩阵的奇异值分解\(\bm{\mathcal{H}}=\bm{U}\bm{\Sigma}\bm{V}^T\)，系统辨识可以认为：

\begin{equation*}
    \begin{cases}
        \bm{\mathcal{O}}=\bm{U}\bm{\Sigma}^{0.5} \\
        \bm{\mathcal{C}}=\bm{\Sigma}^{0.5}\bm{V}^T
    \end{cases}
\end{equation*}

其中\(\bm{\Sigma}^{0.5}\)表示对奇异值逐个元素取平方根。

求取得到能控性矩阵和能观性矩阵后，根据其定义和确定的系统阶次\(k\)，可以得到
\textbf{输入矩阵\(\bm{B}\)由能控性矩阵\(\bm{\mathcal{C}}\)的第一列前\(k\)个元素构成，
输出矩阵为\(\bm{C}\)由能观性矩阵\(\bm{\mathcal{O}}\)第一行的前\(k\)个元素构成}。

为了计算\(\bm{A}\)，可以构造移位汉克尔矩阵：

\begin{equation*}
    \bm{\mathcal{H'}}=
    \begin{bmatrix}
        g_2 & g_3 & \cdots & g_{\mathrm{n+1}} \\
        g_3 & g_4 & \cdots & g_{\mathrm{n+2}} \\
        \vdots & \vdots & \ddots & \vdots \\
...
\begin{equation*}
    ...
    \begin{bmatrix}
        \bm{CAB} & \bm{CA^2}\bm{B} & \cdots & \bm{CA}^{n}\bm{B} \\
        \bm{CA^2}\bm{B} & \bm{CA}^3\bm{B} & \cdots & \bm{CA}^{n+1}\bm{B} \\
        \vdots & \vdots & \ddots & \vdots \\
        \bm{CA}^{n}\bm{B} & \bm{CA}^{n+1}\bm{B} & \cdots & \bm{CA}^{2n-1}\bm{B} \\
    \end{bmatrix}
    =
    \begin{bmatrix}
        \bm{C}\\\bm{CA}\\\bm{CA}^2\\\bm{\vdots}\\\bm{CA}^{n-1}
    ...
    \begin{bmatrix}
        \bm{B}&\bm{AB}&\bm{A}^2\bm{B}&\bm{\cdots}&\bm{A}^{n-1}\bm{B}
    \end{bmatrix}
     =\bm{\mathcal{O}A\mathcal{C}}
\end{equation*}

因此\(\bm{A'}\)可以计算为：

\begin{equation*}
    \bm{A'}=\bm{\mathcal{O}^{-1}\mathcal{H'}\mathcal{C}^{-1}}
\end{equation*}

而进行截断后，\(k\)阶系统实际的\textbf{状态矩阵\(\bm{A}\)由\(\bm{A'}\)的前\(k\)行\(k\)列的元素构成}。

馈通矩阵\(\bm{D}\)一般由两种做法，第一种是直接忽略视为零矩阵，因为实际系统基本不存在随输入信号立刻改变的分量。第二种是辨识认为
\(\bm{D}=g_{0}\)。一般为了让辨识结果更符合实际，会选择第一种做法。

得到了四个矩阵后，可以计算得到传递函数\(G(z)=\bm{C}(z\bm{I}-\bm{A})^{-1}\bm{B}+\bm{D}\)，至此，基于Ho-Kalman算法的系统辨识就完成了。
