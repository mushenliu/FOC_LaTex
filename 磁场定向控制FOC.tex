...
\end{lstlisting}
\footnote{52-55行将限幅后的输出写回控制器的状态本身是不合理的（后续的速度环同理），因为控制器状态不应该被限幅影响，
    本来为开发过程中的bug。但是后续发现如果不做写回处理，控制器会进入深度饱和状态，电流环运算时间超限导致速度环停止运行（电流环优先级更高）。
    分析可知写回处理可以理解为进入饱和状态（限幅开始）瞬间就锁死了控制器状态，完全没有积累饱和状态；不做写回处理
    会一直积累误差加重饱和状态，最终无法退饱和。但该方法无法进行分析和仿真，只是一种权宜之计，后续还需要纠正抗积分饱和措施。}
...
\end{lstlisting}
\footnote{14-19行的输出限幅写回控制器状态不合理}
采用给定速度值180rpm，速度环输出限幅0.5A。无负载情况下的速度阶跃响应如下图所示（横轴单位ms，纵轴单位rpm）。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]
    {图片/速度阶跃响应图.png}
    \caption{$w_m=300rpm$阶跃响应图}
\end{figure}

通过设置速度环输出限幅，能够确保在堵转情况下电机的安全性。在上述条件下堵转电机，DQ轴电流响应如下图所示（横轴单位ms，纵轴单位A，紫线表示
$I_d$，粉线表示$I_q$）。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]
