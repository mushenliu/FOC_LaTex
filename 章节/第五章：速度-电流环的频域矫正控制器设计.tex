...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=3cm, >=latex']
        ...
        % 测量环节：转矩与转速测量噪声注入（置于主回路之上）
        \node [sum, above of=K, node distance=2.2cm] (sumT) {};
        \node [input, above of=sumT, node distance=0.8cm] (nT) {};
        \node [sum, above of=Mech, node distance=2.2cm] (sumn) {};
        \node [input, above of=sumn, node distance=0.8cm] (nn) {};

        % 主回路（环内为无噪声的理想量 \(T_0\)、\(n_0\)）
        \draw [->] (input) -- node {\(U_{\mathrm{q}}\)} (sum);
        \draw [->] (sum) -- node {\(U_{\mathrm{q}}'\)} (RL);
        \draw [->] (RL) -- node {\(i_{\mathrm{q}}\)} (K);
        \draw [->] (K) -- node [name=t0] {\(T_{0}\)} (Mech);
        \draw [->] (Mech) -- node [name=n0] {\(n_{0}\)} (output);
        \draw [->] (n0) |- (Kf);
        \draw [->] (Kf) -| node[pos=0.9] {\(-\)} node [pos=0.5] {\(E\)} (sum);

        % 转矩测量：\(T_0\) 叠加测量噪声 \(n_{\mathrm{T}}\) 得实测转矩 \(T_{\mathrm{e}}\)
        \draw [->] (t0) |- (sumT);
        \draw [->] (nT) -- node {\(n_{\mathrm{T}}\)} (sumT);
        \draw [->] (sumT) -- node {\(T_{\mathrm{e}}\)} ++(-1cm,0);
        % 转速测量：\(n_0\) 叠加测量噪声 \(n_{\mathrm{n}}\) 得实测转速 \(n\)
        \draw [->] (n0) |- (sumn);
        \draw [->] (nn) -- node {\(n_{\mathrm{n}}\)} (sumn);
        \draw [->] (sumn) -- node {\(n\)} ++(-1cm,0);
    \end{tikzpicture}
    \caption{\(i_{\mathrm{d}}=0\)假设下带测量噪声的Q轴电流-速度系统框图}
...
\begin{equation*}
    \hat{G}_{\mathrm{m}}(e^{\jj\omega})=G_{\mathrm{m}}(e^{\jj\omega})
    [1-\cfrac{\sigma_{\mathrm{T}}^2}{S_{T_{\mathrm{e}}T_{\mathrm{e}}}(e^{\jj\omega})}]
\end{equation*}

其中\(\sigma_{\mathrm{T}}^2\)为测量噪声\(n_{\mathrm{T}}\)的方差，而\(S_{T_{\mathrm{e}}T_{\mathrm{e}}}(e^{\jj\omega})\)
...
其中\(S_{xy}(e^{\jj\omega})\)为互功率谱密度，\(S_{xx}(e^{\jj\omega})\)和\(S_{yy}(e^{\jj\omega})\)为自功率谱密度。
...
\subsubsection{闭环反推的一般性误差分析\label{sec:闭环反推的一般性误差分析}}
一般地，根据闭环传递函数代数反推某一个开环传递函数，本质上是对若干传递函数的代数运算。
开环传递函数\(G_{\mathrm{o}}(s)\)可由多个传递函数模块
计算得到，即\(G_{\mathrm{o}}(s)=\cfrac{\prod\limits_{i=1}^nG_i(s)}{\prod\limits_{j=1}^mG_j(s)}\)。
其中部分传递函数由实测数据辨识得到（如整体的闭环传递函数），部分传递函数由其他辨识的静态变量根据确定的结构得到（如电流环被控对象
\(\cfrac{1}{R_{\mathrm{s}}+L_{\mathrm{q}}s}\)）。

一般地，开环传递函数反推式两边取对数可得：
\begin{equation*}
    \ln G_{\mathrm{o}}(s)=\sum\limits_{i=1}^n\ln G_i(s) - \sum\limits_{j=1}^m\ln G_j(s)=
    \sum\limits_{k=1}^{n+m}\ln G_k(s)
\end{equation*}

对两端求全微分可得误差传播公式：
\begin{equation*}
    \Delta\ln G_{\mathrm{o}}(s)=\sum_{k=1}^{n+m}\cfrac{\partial\ln G_{\mathrm{o}}}{\partial\ln G_k}\Delta\ln G_k(s)
\end{equation*}

相对误差为小量时\(\Delta\ln G_{\mathrm{o}}(s)\approx\cfrac{\Delta G_{\mathrm{o}}(s)}{G_{\mathrm{o}}(s)}\)，
\(\Delta\ln G_k(s)\approx\cfrac{\Delta G_k(s)}{G_k(s)}\)，并且定义对于\(G_k(s)\)的灵敏度函数为
\(S_{G_k}(s)=\cfrac{\partial\ln G_{\mathrm{o}}(s)}{\partial\ln G_k(s)}\)，代入可得：
\begin{equation*}
    \cfrac{\Delta G_{\mathrm{o}}(s)}{G_{\mathrm{o}}(s)}
    =\sum_{k=1}^{n+m}S_{G_k}(s)\cfrac{\Delta G_k(s)}{G_k(s)}
\end{equation*}

由于分析误差时主要关心频域信息，因此取\(s=\jj\omega\)可得：
\begin{equation*}
    \cfrac{\Delta G_{\mathrm{o}}(\jj\omega)}{G_{\mathrm{o}}(\jj\omega)}
    =\sum_{k=1}^{n+m}S_{G_k}(\jj\omega)\cfrac{\Delta G_k(\jj\omega)}{G_k(\jj\omega)}
\end{equation*}


其中灵敏度函数\(S_{G_k}(\jj\omega)\)为频率的复函数，定量刻画反推结果
对第\(k\)个传递函数的放大倍数；\(\cfrac{\Delta G_k(\jj\omega)}{G_k(\jj\omega)}\)为第\(k\)个传递函数的相对误差。
对结构已知、参数测量得到的传递函数，传递函数模块的相对误差可以被进一步拆解为对内部测量参数的相对误差。而对于辨识得到的
传递函数，可以用\ref{eq:估计方差与相干函数}来计算相对误差。

本文的速度环辨识即属上述一般框架的实例：间接法先辨识\(U_{\mathrm{q}}\)到\(n\)的闭环传递函数\(G(s)\)，再代数反推
得到机械部分的开环传递函数\(G_{\mathrm{m}}(s)\)，反推关系为
\begin{equation*}
...
该反推过程会引入误差，其来源可分为三类：反推输入量（辨识得到的\(G(s)\)）的测量偏差、闭环辨识的有噪声相关带来的有偏估计问题，以及
反推式中已辨识参数\(R_{\mathrm{s}}\)、\(L_{\mathrm{q}}\)、\(\psi_{\mathrm{m}}\)的误差。

其一是闭环辨识的噪声相关性问题。反电动势构成的物理闭环使输出测量噪声经反馈回路进入输入通道，使输入信号与输出测量噪声
相关，违背维纳-霍普夫方程无偏估计的前提。此时以测量信号为输入（如直接法以实测重构转矩为输入）的辨识将产生不随数据
长度增加而消失的系统偏差。为了规避这一问题，才选择了闭环传递函数辨识后代数反推的方案。

其二是测量偏差。\(G(s)\)由有限长度的实测数据经维纳-霍普夫方程估计得到脉冲响应序列，再通过Ho-Kalman算法得到解析的传递函数。
尽管可以证明该估计期望是无偏估计，但有限长数据带来随机误差（方差），可以用\ref{eq:估计方差与相干函数}来近似为：
\begin{equation*}
    \cfrac{\sigma_{\hat G}(\omega)}{|G(\jj\omega)|}
    \approx\sqrt{\cfrac{1-\gamma_{xy}^2(\omega)}{\gamma_{xy}^2(\omega)N_{\mathrm{eff}}}}
\end{equation*}

其三，反推式中使用的\(R_{\mathrm{s}}\)、\(L_{\mathrm{q}}\)、\(\psi_{\mathrm{m}}\)为已辨识参数，各自携带辨识误差。
其误差经模块对应的灵敏度函数传播后表现出频率相关性，在不同频段贡献不同的相对误差。

此外，反推式分母中的\(1-\cfrac{\pi}{30}p\psi_{\mathrm{m}}G(s)\)在低频段因环路增益较大而趋近于零，上述各类误差会被进一步放大。

\subsubsection{反推结果的敏感性分析与辨识模型求解\label{sec:反推结果的敏感性分析}}
下面以上述误差为输入，给出反推结果误差的定量表达式。对上述反推关系取对数可得：
\begin{equation*}
...
\begin{equation}
    \begin{cases}
        S_{R}(\jj\omega)=\cfrac{R_{\mathrm{s}}}{R_{\mathrm{s}}+\jj\omega L_{\mathrm{q}}}\\
        S_{L}(\jj\omega)=\cfrac{\jj\omega L_{\mathrm{q}}}{R_{\mathrm{s}}+\jj\omega L_{\mathrm{q}}}\\
        S_{G}(\jj\omega)=\cfrac{1}{1-\cfrac{\pi}{30}p\psi_{\mathrm{m}}G(\jj\omega)}\\
        S_{\psi}(\jj\omega)=\cfrac{\cfrac{\pi}{15}p\psi_{\mathrm{m}}G(\jj\omega)-1}{1-\cfrac{\pi}{30}p\psi_{\mathrm{m}}G(\jj\omega)}
    \end{cases}
    \label{eq:灵敏度函数}
\end{equation}
...
那么相对误差可以表示为：
\begin{equation*}
    \cfrac{\Delta G_{\mathrm{m}}(\jj\omega)}{G_{\mathrm{m}}(\jj\omega)}=
    S_{R}(\jj\omega)\cfrac{\Delta R_{\mathrm{s}}}{R_{\mathrm{s}}}
    +S_{L}(\jj\omega)\cfrac{\Delta L_{\mathrm{q}}}{L_{\mathrm{q}}}
    +S_{G}(\jj\omega)\cfrac{\Delta G}{G(s)}
    +S_{\psi}(\jj\omega)\cfrac{\Delta \psi_{\mathrm{m}}}{\psi_{\mathrm{m}}}
\end{equation*}
...
\begin{equation}
    \cfrac{\sigma_{G_{\mathrm{m}}}(\omega)}{|G_{\mathrm{m}}(\jj\omega)|}
    =|S_R(\jj\omega)|\cfrac{\sigma_R}{R_{\mathrm{s}}}
    +|S_L(\jj\omega)|\cfrac{\sigma_L}{L_{\mathrm{q}}}
    +|S_G(\jj\omega)|\cfrac{\sigma_G(\omega)}{|G(\jj\omega)|}
    +|S_\psi(\jj\omega)|\cfrac{\sigma_\psi}{\psi_{\mathrm{m}}}
    \label{eq:标准差上界}
\end{equation}
...
\(\cfrac{\sigma_G(\omega)}{|G(\jj\omega)|}\)为闭环辨识的相对误差，由相干函数按\eqref{eq:估计方差与相干函数}估计。
...
极大的偏离\footnote{直流处灵敏度函数\(S_G(\jj0)=[1-\cfrac{\pi}{30}p\psi_{\mathrm{m}}G(0)]^{-1}\approx 22\)，
