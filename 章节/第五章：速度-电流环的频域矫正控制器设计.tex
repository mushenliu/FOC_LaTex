...
由于分析误差时主要关心频域信息，因此取\(s=\mathrm{j}\omega\)可得：
\begin{equation*}
    \cfrac{\Delta G_{\mathrm{o}}(\mathrm{j}\omega)}{G_{\mathrm{o}}(\mathrm{j}\omega)}
    =\sum_{k=1}^{n+m}S_{G_k}(\mathrm{j}\omega)\cfrac{\Delta G_k(\mathrm{j}\omega)}{G_k(\mathrm{j}\omega)}
\end{equation*}


其中灵敏度函数\(S_{G_k}(\mathrm{j}\omega)\)为频率的复函数，定量刻画反推结果
对第\(k\)个传递函数的放大倍数；\(\cfrac{\Delta G_k(\mathrm{j}\omega)}{G_k(\mathrm{j}\omega)}\)为第\(k\)个传递函数的相对误差。
