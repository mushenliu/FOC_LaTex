...
通过\(i_{\mathrm{C}}=-i_{\mathrm{A}}-i_{\mathrm{B}}\)来计算。考虑到被控设备为云台电机，标称额定电流0.9A，因此选择5mΩ作为电流检测电阻，比例放大电路放大系数配置为100。
...
\eqref{eq:三相占空比表达式}可以发现，计算三相占空比依赖母线电压\(U_{\mathrm{dc}}\)。对于云台电机，工作电压范围为12V到24V，母线电压差距较大。为了
...
母线电压检测电路如下图\ref{fig:母线电压检测电路}所示。通过\(R_{\mathrm{4}},R_{\mathrm{5}}\)实现\(\cfrac{1.33\mathrm{k}\Omega}{10\mathrm{k}\Omega+1.33\mathrm{k}\Omega}=0.1173\)
的分压采集。在ADC最大输入电压3.3V情况下允许供电电压为\(3.3\mathrm{V}\div0.1173=28.13\mathrm{V}\)，足以覆盖电机供电电压。
通过\(R_{\mathrm{7}},C_{\mathrm{6}}\)构成RC低通滤波电路，上限截止频率为\(\cfrac{1}{2\pi\times20\mathrm{k}\Omega\times22\mu\mathrm{F}}=0.36\mathrm{Hz}\)。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]
    {图片/母线电压检测电路.png}
...
然后将OPAMP1和OPAMP2外设的输出端连接到ADC外设，分别为ADC1\_IN3通道和ADC2\_IN3通道，将放大后的电压通过ADC采集实现电流检测。

为了实现母线电压检测，可以将母线电压检测电路的输出端连接到OPAMP3外设，并且将OPAMP3外设配置为跟随器模式，进一步提升输入阻抗，避免输入电流
带来的压降影响对母线电压的判断。同样地，将OPAMP3外设输出端连接到ADC外设，为ADC1\_IN12通道。
...
4 Event）。也就是说，每当TIM4\_CH4产生一次捕获比较事件后就会通过硬件直接触发注入组ADC采集，避免中断带来的软件延迟。

此外，由于ADC1\_IN3通道和ADC2\_IN3负责检测电机相电流，对于精度有较高的要求。而通过电阻分压得到的1.65V偏置电压会因为分压电阻
...
\(f_{\mathrm{PWM}}=\cfrac{170\mathrm{MHz}}{2(0+1)(5664+1)}\approx 15.004\mathrm{kHz}\)，对应的电流环控制周期为66.647us。
使能TIM1\_CH1、TIM1\_CH2和TIM1\_CH3为PWM生成模式，用于产生三相PWM；
使能TIM1\_CH4为PWM生成无输出模式，将捕获比较寄存器CCR配置为ARR-1，用于产生捕获比较事件和捕获比较中断以开始电流环控制。

采用TIM2作为速度环控制中断的定时器，预分频器PSC设置为0，计数模式为向上计数，自动重装寄存器ARR设置为170000-1，此时更新中断频率为
\(f_{\mathrm{INT}}=\cfrac{170\mathrm{MHz}}{(0+1)(170000-1+1)}= 1\mathrm{kHz}\)，对应的速度环控制周期为1ms。
...
首先是对磁编码器TLE5012B的驱动文件撰写。定义头文件\texttt{MY\_TLE5012B.h}，包含指令码宏定义、片选信号产生与解除宏函数定义
和驱动函数声明：
...
定义源文件\texttt{MY\_TLE5012B.c}，主要是编写驱动程序的具体函数实现：
...
在函数\texttt{uint16\_t TLE5012B\_SPI\_Read(uint16\_t Order\_Word)}中，采用阻塞式SPI发送和接收函数，主要是因为在
半双工模式下需要手动切换目前SPI的状态是发送还是接收，不便于使用DMA。此外\texttt{double TLE5012B\_Speed(void)}函数
仅作为驱动测试，实际控制中由于读取一次数据花费时间较长，对于速度的计算一般通过角度之差实现。

另外需要编写与上位机通信相关的函数。为了减少通信占用的事件，加快通信频率，选择每一次速度环控制周期进行一次数据上传。
通信协议采用JUSTFLOAT协议，只传递\texttt{float}类型的数据。定义头文件\texttt{MY\_JUSTFLOAT.h}和源文件\texttt{MY\_JUSTFLOAT.c}
如下所示。
...
黑色部分所示，红色部分是控制器所在的坐标系（控制坐标系），两者存在电角度偏差\(\Delta \theta\)满足
\(\theta_{\mathrm{real}}=\theta_{\mathrm{control}}+\Delta\theta\)
。设\(U_{\mathrm{d}},U_{\mathrm{q}}\)表示实际坐标系的驱动电压，
而\(U_{\mathrm{d_{\mathrm{c}}}},U_{\mathrm{q_{\mathrm{c}}}}\)表示控制坐标系下的驱动电压。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]
    {图片/控制坐标系和实际坐标系.png}
...
根据电磁转矩表达式\eqref{eq:同步坐标系下电磁转矩表达式（一般形式）}，可以发现\(T_{\mathrm{e}}\propto i_{\mathrm{q}}\)。而根据电路方程
\eqref{eq:同步坐标系下电路方程}可知，在初始无角速度情况下，只有通过给定\(U_{\mathrm{q}}\)，才能使电机产生电磁转矩，使得电机旋转。
若只给定\(U_{\mathrm{d}}\)，则电机不会旋转。根据图\ref{fig:控制坐标系和实际坐标系}可知，在给定\(U_{\mathrm{d_{\mathrm{c}}}}\)的情况下，编码器零位校准正确
的判别方法就是电机不发生旋转。
因此最简单的方法就是从零开始，按照特定步长递增，尝试每一个可能的\(\Delta\theta\)，直到电机在给定\(U_{\mathrm{d}}\)持续一定时间
...
假设当\(U_{\mathrm{q}}>0\)时产生的电磁转矩在空载情况下使电机旋转的方向为正向。那么从0°开始不断增大尝试的电角度偏差，会经历反转减慢、正转加快、正转减慢
...
可以从另一个角度分析，不妨假设这一过程为闭环控制：期望角速度为0，根据反馈得到的角速度偏差（\(-\omega_{\mathrm{m}}\)），
执行特定控制律输出尝试的编码器零位，直到给定\(U_{\mathrm{d}}\)运行一段时间电机角度基本不发生变化。

为了方便起见，假设控制器为I控制器，即下一步尝试的编码器零位满足\(\Delta\theta_{\mathrm{next}}=\Delta\theta_{\mathrm{current}}-K_{\mathrm{i}}\omega_{\mathrm{m}}\)。
...
电流内环PI控制由TIM1\_CH4的比较捕获事件/中断触发，主要包含数据采集、克拉克-帕克变换求取\(I_{\mathrm{d}},I_{\mathrm{q}}\)、DQ轴PID控制运算、SVPWM调制、设定
三相占空比几个步骤。
...
在控制函数中，主要执行计算电角度、进行克拉克-帕克变换求取当前的\(I_{\mathrm{d}},I_{\mathrm{q}}\)、进行PID控制计算、SVPWM调制、设定三相占空比几个步骤。
...
采用PID控制器。在连续域中的传递函数为\(G_{\mathrm{PID}}(s)=K_{\mathrm{p}}+\cfrac{K_{\mathrm{i}}}{s}+K_{\mathrm{d}}\cfrac{s}{\frac 1N s+1}\)，采用双线性变换
\(s=\cfrac{2}{T_{\mathrm{s}}}\cfrac{1-z^{-1}}{1+z^{-1}}\)，接下来推导采用双线性变换后的离散域传递函数与差分方程表示。

对于比例项保持不变为\(K_{\mathrm{p}}\)，积分项双线性变换后为\(\cfrac{K_{\mathrm{i}}T_{\mathrm{s}}}{2}\times\cfrac{1+z^{-1}}{1-z^{-1}}\)

微分项的双线性变换后为\(K_{\mathrm{d}}\times\cfrac{2}{T_{\mathrm{s}}}\cfrac{1-z^{-1}}{1+z^{-1}}\times\cfrac{1}{\frac{1}{N}\times
\frac{2}{T_{\mathrm{s}}}\frac{1-z^{-1}}{1+z^{-1}}+1}\)，
展开得到\(\cfrac{2K_{\mathrm{d}}}{T_{\mathrm{s}}}\times\cfrac{NT_{\mathrm{s}}(1-z^{-1})}{2(1-z^{-1})+NT_{\mathrm{s}}(1+z^{-1})}\)，化简可得
\(\cfrac{2K_{\mathrm{d}}N}{NT_{\mathrm{s}}+2}\times\cfrac{1-z^{-1}}{1+\frac{NT_{\mathrm{s}}-2}{NT_{\mathrm{s}}+2}z^{-1}}\)。

定义离散域下的系数\(P=K_{\mathrm{p}},I=\cfrac{K_{\mathrm{i}}T_{\mathrm{s}}}{2},D=\cfrac{2K_{\mathrm{d}}N}{NT_{\mathrm{s}}+2},N_{\mathrm{d}}=\cfrac{NT_{\mathrm{s}}-2}{NT_{\mathrm{s}}+2}\)，则总的传递函数可以表示为
\begin{align*}
    G(z)&=P+I\cfrac{1+z^{-1}}{1-z^{-1}}+D\cfrac{1-z^{-1}}{1+N_{\mathrm{d}}z^{-1}}\\
    & = \cfrac{P(1-z^{-1})(1+N_{\mathrm{d}}z^{-1})+
    I(1+z^{-1})(1+N_{\mathrm{d}}z^{-1})+
    D(1-z^{-1})^2}{(1-z^{-1})(1+N_{\mathrm{d}}z^{-1})}\\
    & =\cfrac{(P+I+D)+(PN_{\mathrm{d}}-P+IN_{\mathrm{d}}+I-2D)z^{-1}+(D+IN_{\mathrm{d}}-PN_{\mathrm{d}})z^{-2}}{1+(N_{\mathrm{d}}-1)z^{-1}-N_{\mathrm{d}}z^{-2}}\\
    & = \cfrac{Y(z)}{E(z)}
\end{align*}
...
\begin{align*}
    G'(z)&=(1-z^{-1})G(z)\\
    & = \cfrac{(P+I+D)+(PN_{\mathrm{d}}-2P+IN_{\mathrm{d}}-3D)z^{-1}+(3D-2PN_{\mathrm{d}}+P-I)z^{-2}+(PN_{\mathrm{d}}-D-IN_{\mathrm{d}})z^{-3}}{1+(N_{\mathrm{d}}-1)z^{-1}-N_{\mathrm{d}}z^{-2}}\\
    & = \cfrac{b_{0}+b_{1}z^{-1}+b_{2}z^{-2}+b_{3}z^{-3}}{1+a_{1}z^{-1}+a_{2}z^{-2}}\\
    & = \cfrac{\Delta Y(z)}{E(z)}
\end{align*}
...
SVPWM调制按照\eqref{eq:零序分量表达式}和\eqref{eq:三相占空比表达式}计算即可。但根据电压极限圆表达式\eqref{eq:SVPWM电压极限圆}，
需要检查给定的\(U_{\mathrm{d}},U_{\mathrm{q}}\)是否在电压极限圆内。如果不在电压极限圆内，需要进行线性放缩。相关函数如下所示。
...
最后调用上述函数的控制函数\texttt{void Current\_Control()}如下所示。
...
使用额外的抗积分饱和手段。
通过给定\(I_{\mathrm{d}}\)检测电流闭环控制效果。在给定\(I_{\mathrm{d}}=0.2\mathrm{A}\)和\(I_{\mathrm{d}}=-0.2\mathrm{A}\)情况下电流响应如下图所示。

\begin{figure}[H]
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=1.1\textwidth]{图片/Id=0.2A阶跃响应图.png}
        \caption{\(I_{\mathrm{d}}\)=0.2A阶跃响应图}
    \end{minipage}
    \begin{minipage}{0.5\textwidth}
        \centering
        \includegraphics[width=1.1\textwidth]{图片/Id=-0.2A阶跃响应图.png}
...
在速度环控制函数中，主要是执行速度环PID运算。采用\(I_{\mathrm{d}}=0\)的控制策略，则速度环的输出就是Q轴电流环的给定值。为了避免堵转时速度环控制输出过高
...
\begin{figure}[H]
    ...
    {图片/速度阶跃响应图.png}
    \caption{\(n_{\mathrm{m}}=300\mathrm{rpm}\)阶跃响应图}
\end{figure}

通过设置速度环输出限幅，能够确保在堵转情况下电机的安全性。在上述条件下堵转电机，DQ轴电流响应如下图所示（紫线表示
\(I_{\mathrm{d}}\)，粉线表示\(I_{\mathrm{q}}\)）。
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]
    {图片/DQ电流响应图.png}
